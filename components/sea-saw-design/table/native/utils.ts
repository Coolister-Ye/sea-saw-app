...
/* ─────────────────────────────────────────────────────────────────────────
   Re-exports from grid/utils — kept here for backward compatibility with
   old NativeTable components (useColumnModel, TableBody, etc.)
   ───────────────────────────────────────────────────────────────────────── */

export { applyFlexWidths, getRowKey } from "@/components/sea-saw-design/grid/utils";
...
export function buildColumns(
  ...
  const metaCols: ComputedColumn[] = Object.entries(meta)
    ...
    .map(([field, fieldMeta]) => {
      ...
      return {
        ...
        suppressMenu: custom.suppressMenu,
        suppressAutoSize: custom.suppressAutoSize,
        resizable: custom.resizable,
        fieldMeta,
      };
    });
  ...
  const extraCols: ComputedColumn[] = Object.entries(colDefinitions ?? {})
    ...
    .map(([field, def]) => ({
      ...
      suppressAutoSize: def.suppressAutoSize,
