...
export default function AutoCreateOrderToggle({
  ...
  const iconContainerClass = checked
  ...
  return (
    <View
      className={`mt-3 p-4 rounded-xl border ${containerClass} ${disabled ? "opacity-50" : ""}`}
    >
      <View className="flex-row items-center justify-between">
        <View className="flex-row items-center flex-1">
          <View
            className={`w-10 h-10 rounded-lg mr-3 items-center justify-center ${iconContainerClass}`}
            style={{ boxShadow: iconShadow, elevation: 3 }}
          >
          ...
          <View className="flex-1">
            <View className="flex-row items-center gap-1.5">
              <Text
                className={`font-semibold text-sm ${checked ? "text-indigo-900" : "text-slate-700"}`}
              >
            ...
            <Text
              className={`text-xs mt-0.5 ${checked ? "text-indigo-600" : "text-slate-500"}`}
            >
              {checked ? (
        ...
        <Switch
          ...
          onChange={onChange}
          disabled={disabled}
          style={{ backgroundColor: checked ? "#6366f1" : undefined }}
        />
      </View>
