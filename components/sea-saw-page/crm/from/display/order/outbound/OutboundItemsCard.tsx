...
export default function OutboundItemsCard({
  ...
  if (!value?.length) {
    return <EmptySlot message={i18n.t("No outbound items")} />;
  }
      ...
      {value.map((item, index) => {
        ...
        return (
          ...
          >
            ...
            {/* ================= Outbound Info ================= */}
            <InfoSection title={i18n.t("Outbound Information")}>
              <View className="flex-row gap-4 flex-wrap">
                <InfoField
                  label={getLabel("outbound_qty")}
                  value={item.outbound_qty}
                  format={formatNumberTrim}
                />
                <InfoField
                  label={getLabel("outbound_net_weight")}
                  value={item.outbound_net_weight}
                  format={formatNumberTrim}
                  suffix={weightUnit}
                />
                <InfoField
                  label={getLabel("outbound_gross_weight")}
                  value={item.outbound_gross_weight}
                  format={formatNumberTrim}
                  suffix={weightUnit}
                />
              </View>
