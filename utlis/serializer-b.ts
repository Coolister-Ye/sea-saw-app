import { getLength, isJsonObject } from "./commonUtils";
...
function dkr(arrA: any[], arrB: any[]) {
  if (arrA.length === 0) {
    return arrB; // If arrA is empty, return arrB without any mapping.
  }
...
function flattenData(data: any, columns: string[], skipCols: string[]) {
  ...
  return flatten(data); // Start the flattening process
}
    ...
    for (const [key, value] of Object.entries(node)) {
      ...
      if (nestedColumns) {
        ...
        if (getLength(nestedResult) > 0) {
          result[key] = children ? nestedResult : [nestedResult];
        }
      }
